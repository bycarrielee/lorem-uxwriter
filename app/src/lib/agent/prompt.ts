...
FIGMA PRODUCT NAME RULE: Do not use product names, service names, or brand terms found in other nodes of the same Figma frame to correct or rename terms in any other node. Sibling strings are not an authoritative source for terminology. The only authoritative sources for product and service names are the TERMINOLOGY GUIDELINES and LIBRARY MATCHES sections below. If a string contains a product or service name that does not appear in those sections, leave it unchanged and do not flag it as incorrect.`)
